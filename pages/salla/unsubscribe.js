...
const SallaUnsubscribePage = () => {
  ...
  const callbackUrl = `${process.env.NEXT_PUBLIC_CLIENT_BASE_URL}/salla/unsubscribe`
  ...
  useEffect(() => {
    ...
    const unsubscribeToSallaWebhook = async () => {
      try {
        const config = {
          headers: {
        ...
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_DOMAIN}/Dashboard/Merchant/v1.0/salla/unsubscribe`,
          '',
          config
        )
    ...
    <div className="flex items-center justify-center w-full h-full min-h-screen gap-2">
      {isLoading && (
        <div className="flex flex-col items-center justify-center gap-2 text-black dark:text-white">
          ...
          <p className="text-xl">Disconnecting your store...</p>
...
export default SallaUnsubscribePage
