...
import d from './assets/3.svg'

const OrderPushTemplate = ({ templateData }) => {
  const {
    selectedLang,
    isLoading,
    setIsLoading,
  ...
  return (
    <div className="w-full max-w-[400px] flex flex-col items-center justify-start gap-5">
      <p className="text-lg font-bold text-BlackSec">
        {' '}
        {t('Template_orderpush')}{' '}
      </p>
      <div className="flex flex-col items-center relative justify-between w-full bg-white dark:bg-bgBlack rounded-[10px] p-4  h-full min-h-[700px]">
        <div className="flex flex-col items-start justify-start w-full gap-3">
          ...
          />
          {selectedLang == 'en'
            ? templateData?.en?.order_push.data.map((item, i) => {
              return i == templateData?.en?.order_push.header ? (
                editMode === true ? (
                  <textarea
                    key={item}
                  ...
                  <p
                    key={item}
                    className={`${(i == 0 || templateData.en.order_push.footer == i) &&
                      'font-bold'
                      }`}>
                    {header.en}
                  </p>
                )
              ) : i == templateData?.en?.order_push.footer ? (
                editMode === true ? (
                  <textarea
                    key={item}
            ...
            : templateData?.ar?.order_push.data.map((item, i) => {
              return i == templateData?.ar?.order_push.header ? (
                editMode === true ? (
                  <textarea
                    key={item}
                    name="orderInitHeader"
                  ...
                  <p
                    key={item}
                    className={`${(i == 0 || templateData.ar.order_push.footer == i) &&
                      'font-bold'
                      }`}>
                    {header.ar}
                  </p>
                )
              ) : i == templateData?.ar?.order_push.footer ? (
                editMode === true ? (
                  <textarea
                    key={item}
                    name="orderInitFooter"
        ...
        <div className="flex flex-col items-center justify-start w-full gap-2">
          {templateData?.en?.order_push.button.map((item, i) => {
            return (
              <button
                key={i}
                className={`shadow-xs text-sm font-bold ${i == 0 && 'text-Blue'
                  } ${i == 1 && 'text-green-500'} ${i == 2 && 'text-red-500'
...
export default OrderPushTemplate
