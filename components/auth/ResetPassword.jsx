    ...
    if (
      ...
      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_DOMAIN}/public/v1.0/credentials/reset?token=${token}`,
          '',
          config
        )
        resetErrors()
    ...
    const validate = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_DOMAIN}/public/v1.0/link/validator/reset_password?token=${token}`
          // config
        )
        if (res.data.Status) {
          resetErrors()
  ...
  return (
    <div className="flex items-center justify-center w-screen">
      ...
      <div className="flex flex-col items-center justify-center w-full h-full min-h-screen pt-10 pb-3 bg-white">
        <div className="flex flex-col items-center justify-center w-full max-w-md gap-5 px-4 pb-20 scale-90 tabletM:pb-0 tabletM:gap-7 laptop:px-0">
          <div className="flex flex-col items-center gap-4 text-Black">
            <p className="text-2xl font-[800]">
              {t && t('Auth_signup_Setup_Password')}
            </p>
            <p className="text-lg text-center text-BlackSec">
              {t && t('Auth_password_reset_info')}
            </p>
          </div>
